...
const EditCompany = () => {
    ...
    const showBranchesDiv = program => {
        if(programsDiv[program.toLowerCase()]) {
            setNewCompanyData({ ...newCompanyData, "eligibility": {
                ...newCompanyData["eligibility"], [program]: {}
            }})
        }
    ...
    const postCompanyDetails = (e) => {
        e.preventDefault();
        dispatch(editCompany(newCompanyData));
    };
        ...
        if(company) {
            ...
            programs.forEach(program => {
                if(company.eligibility) {
                    if(!company.eligibility[program] || Object.keys(company.eligibility[program]).length === 0) company.eligibility[program] = {};
                    else showBranchesDiv(program)
                }
                if(company.package) {
                    if(!company.package[program] || Object.keys(company.package[program]).length === 0) company.package[program] = {};
                }
            })
            let temp = {...newCompanyData};
            for (const key in company) {
                temp[key] = company[key];
            }
            setNewCompanyData(temp);
        }
    }, [company])
        ...
        <>
            <div className="row page-titles">
                <div className="col-md-5 align-self-center">
                    <h4 className="text-themecolor">Edit Company Details</h4>
                </div>
            </div>


            {/* Add New Company Form */}
            {!successMsg && company && <form onSubmit={postCompanyDetails}>
                <div className="row">
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-md-6">
                                        <div className="form-group">
                                            ...
                                            {newCompanyData.joining_date && <input type="date" className="form-control" name="joining_date" value={new Date(newCompanyData.joining_date).toISOString().substring(0, 10)} onChange={handleCompanyDataChange} />}
                                        </div>
                                    </div>
                                </div>
                                <div className="row">
                                                ...
                                                {programs.map((program, idx) => (
                                                    <div className="col-md-2" key={idx}>
                                                        <label className="custom-control custom-checkbox">
                                                            <input type="checkbox" className="custom-control-input" checked={programsDiv[program.toLowerCase()]} onChange={(e) => showBranchesDiv(program, e)} />
                            ...
                            <div className="card-body">
                                ...
                                <div className="row">
                                    <div className="col-md-6">
                                        ...
                                        {newCompanyData.deadline_date && <input type="datetime-local" className="form-control" placeholder="Enter date" name="deadline_date" value={new Date(newCompanyData.deadline_date).toISOString().substring(0, 16)} onChange={handleCompanyDataChange} required />}
                                    </div>
                                </div>
                            </div>
                        </div>
