...
export const addNewAnnouncement = (announcement) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post("https://placement-portal-liart.vercel.app/api/announcement/add", announcement, config);
...
export const getAllAnnouncementsStudent = () => async (dispatch) => {
    try{
        ...
        const {data} = await axios.post('https://placement-portal-liart.vercel.app/api/announcement/getAll');
...
export const getAllAnnouncementsAdmin = (passout_batch) => async (dispatch) => {
    try{
        ...
        const {data} = await axios.post('https://placement-portal-liart.vercel.app/api/announcement/getAll', {passout_batch}, config);
        dispatch({ type: GET_ANNOUNCEMENTS_SUCCESS, payload: data });
    } catch(error) {
        dispatch({
            type: GET_ANNOUNCEMENTS_FAIL,
