...
export const getApplicationStatus = (company_id) => async (dispatch) => {
    try{
        ...
        const {data} = await axios.get(`/api/apply/getStatus/${company_id}`);
...
export const apply = (company_id) => async (dispatch) => {
    try{
        ...
        const {data} = await axios.post("/api/apply/apply", {company_id}, config);
...
export const withdrawRegistrationAdmin = (college_id, company_id) => async (dispatch) => {
    try{
        ...
        const {data} = await axios.post("/api/apply/withdrawOne", {college_id, company_id}, config);
...
export const withdrawRegistrationStudent = (company_id) => async (dispatch) => {
    try{
        ...
        const {data} = await axios.post("/api/apply/withdraw", {company_id}, config);
        dispatch({ type: WITHDRAW_SUCCESS, payload: data });
    } catch(error) {
        dispatch({
            type: WITHDRAW_FAIL,
